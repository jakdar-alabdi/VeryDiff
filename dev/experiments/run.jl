...
function verydiff_epsilon(nn_file₁::String, nn_file₂::String, spec_file::String, epsilon::Float64, timeout::Int64, result_out_dir::String; save=true)
    ...
    set_neuron_splitting_config((false, false, false))
...
function deepsplit_epsilon(config::Tuple{Bool, Bool, Bool}; mode=ZonoBiased, approach=LP, contract=ZonoContract)
...
function deepsplit_top1(config::Tuple{Bool, Bool, Bool}; mode=ZonoBiased, approach=LP, contract=ZonoContract)
...
function run_experiments_pool4()
    ...
    run_func(deepsplit_epsilon((true, false, false); mode=DeepSplitUnbiased, approach=ZonoContraction, contract=LPZonoContract), "LP-ZC-DU")
    run_func(deepsplit_epsilon((true, true, true); mode=DeepSplitUnbiased, approach=ZonoContraction, contract=LPZonoContract), "LP-ZC-DU-Input-DiffZono")

    run_func(deepsplit_epsilon((true, false, false); mode=DeepSplitUnbiased, approach=ZonoContraction, contract=ZonoContractInter), "ZCInter-DU")
    run_func(deepsplit_epsilon((true, true, true); mode=DeepSplitUnbiased, approach=ZonoContraction, contract=ZonoContractInter), "ZCInter-DU-Input-DiffZono")

    run_func(deepsplit_epsilon((true, false, false); mode=DeepSplitUnbiased, approach=ZonoContraction, contract=ZonoContractPost), "ZCPost-DU")
    run_func(deepsplit_epsilon((true, true, true); mode=DeepSplitUnbiased, approach=ZonoContraction, contract=ZonoContractPost), "ZCPost-DU-Input-DiffZono")

    run_func(deepsplit_epsilon((true, false, false); mode=DeepSplitUnbiased, approach=ZonoContraction, contract=ZonoContract), "ZC-DU")
    run_func(deepsplit_epsilon((true, true, true); mode=DeepSplitUnbiased, approach=ZonoContraction, contract=ZonoContract), "ZC-DU-Input-DiffZono")

    run_func(deepsplit_epsilon((true, false, false); mode=DeepSplitUnbiased, approach=LP), "LP-DU")
    run_func(deepsplit_epsilon((true, true, true); mode=DeepSplitUnbiased, approach=LP), "LP-DU-Input-DiffZono")

    run_func(deepsplit_epsilon((true, false, false); mode=DeepSplitUnbiased, approach=VerticalSplitting), "VS-DU")
    run_func(deepsplit_epsilon((true, true, true); mode=DeepSplitUnbiased, approach=VerticalSplitting), "VS-DU-Input-DiffZono")
    
    run_func(verydiff_epsilon, "VeryDiff")
end
