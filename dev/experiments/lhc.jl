...
function _run_mnist_all_epsilon(specs_csv_file::String, warmup_specs_csv_file::String, log_dir::String, run_name::String, eval_func)
    ...
    open(specs_csv_file, "r") do f
        while !eof(f)
            ...
            mnist_name = "lhc-$epsilon-$timeout"
...
function _run_mnist_all_top1(specs_csv_file::String, warmup_specs_csv_file::String, log_dir::String, run_name::String, eval_func)
    ...
    open(specs_csv_file, "r") do f
        while !eof(f)
            ...
            net_name = replace(basename(nn_file₂), ".onnx" => "")
            mnist_name = "lhc-top1-$delta-$timeout"
            out_dir = joinpath(log_dir, run_name, mnist_name, net_name)
            mkpath(out_dir)
            spec_file_name = replace(basename(spec_file), ".vnnlib" => "")
            log_file_name = joinpath(out_dir, "$spec_file_name.log")
...
function run_mnist_all_epsilon(eval_func, run_name::String)
    _run_mnist_all_epsilon("$cur_dir/specs/lhc.csv", "$cur_dir/specs/lhc_warmup.csv", "$cur_dir/experiments_final", run_name, eval_func)
end

function run_mnist_all_top1(eval_func, run_name::String)
    _run_mnist_all_top1("$cur_dir/specs/lhc.csv", "$cur_dir/specs/lhc_warmup.csv", "$cur_dir/experiments_final", run_name, eval_func)
end
