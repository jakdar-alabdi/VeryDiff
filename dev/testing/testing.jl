...
function verydiff_epsilon()
    return (nn_file₁::String, nn_file₂::String, spec_file::String, epsilon::Float64, timeout::Int64, result_out_dir::String; save=true) -> begin
        ...
        for (bounds, _, _, _) in f
            veri_result = verify_network(N₁, N₂, bounds, property_check, epsilon_split_heuristic; timeout=timeout, fuzz_testing=fuzz_testing)
            println(veri_result)
        end
    end
end
...
function deepsplit_epsilon(config::Tuple{Bool, Bool, Bool}; mode=VeryDiff.ZonoBiased, approach=VeryDiff.LP, contract=VeryDiff.ZonoContract)
    return (nn_file₁::String, nn_file₂::String, spec_file::String, epsilon::Float64, timeout::Int64, result_out_dir::String; save=true) -> begin
        N₁, N₂ = parse_networks(nn_file₁, nn_file₂)
        f, n_inputs, _ = get_ast(spec_file)
...
function deepsplit_top1(config::Tuple{Bool, Bool, Bool}; mode=VeryDiff.ZonoBiased, approach=VeryDiff.LP, contract=VeryDiff.ZonoContract)
    return (nn_file₁::String, nn_file₂::String, spec_file::String, delta::Float64, timeout::Int64, result_out_dir::String; save=true) -> begin
        N₁, N₂ = parse_networks(nn_file₁, nn_file₂)
        f, n_inputs, _ = get_ast(spec_file)
        VeryDiff.set_neuron_splitting_config(config; mode=mode, approach=approach, contract=contract)
...
# verifier = deepsplit_top1((true, true, true); mode=VeryDiff.DeepSplitUnbiased, approach=VeryDiff.ZonoContraction, contract=VeryDiff.ZonoContractInter)
